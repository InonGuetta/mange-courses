      ...
      <TableBody>
        {myCourses?.length > 0 ? (
          myCourses.map((item, idx) => {
            ...
            const teacherName = teacher?.name ?? teacher?.teacherName ?? (teacherId != null ? `ID: ${teacherId}` : `ID: ${item.course_id}`);
            const studentName = student?.name ?? student?.studentName ?? `ID: ${currentStudentId}`;
            return (
              <TableRow key={item.id} sx={getTableRowSx(idx)}>
                <TableCell component="th" scope="row" sx={tableFirstCellSx}>{idx + 1}</TableCell>
                <TableCell sx={tableBodyCellSx}>{course?.name_course ?? item.course_id}</TableCell>
                <TableCell sx={tableBodyCellSx}>{course?.detail ?? item.detail ?? "-"}</TableCell>
          ...
          <TableRow>
            <TableCell colSpan={6} align="center" sx={emptyRowCellSx}>
              <Typography variant="body1" color="text.secondary">courses not found</Typography>
            </TableCell>
          </TableRow>
        )}
      </TableBody>
