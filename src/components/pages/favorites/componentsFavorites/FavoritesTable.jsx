...
import {
  tableContainerSx, tableHeaderRowSx, tableHeaderCellSx,
  tableBodyCellSx, getTableRowSx
        ...
        {favorites.length > 0 ? favorites.map((item, idx) => {
          ...
          return (
            <TableRow key={item.id} sx={getTableRowSx(idx)}>
              <TableCell component="th" scope="row" sx={{ ...bodyCellCentered, fontWeight: 600 }}>{idx + 1}</TableCell>
              ...
              <TableCell sx={{ ...bodyCellCentered, wordWrap: 'break-word', overflowWrap: 'break-word', whiteSpace: 'normal' }}>
                {course?.detail ?? '-'}
              </TableCell>
